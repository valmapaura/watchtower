...
export default function SettingsPage() {
  ...
  const handleSaveDirChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    setSettings((prev) => (prev ? { ...prev, output_dir: e.target.value } : prev));
  };
            ...
            >
              <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                ...
                <div className="sm:col-span-2">
                  <Field label="Save clips to this folder">
                    <input
                      type="text"
                      value={settings.output_dir}
                      onChange={handleSaveDirChange}
                      placeholder="e.g. D:\CameraRecordings"
                      className="w-full rounded-lg border border-zinc-700 bg-zinc-950 px-3 py-2 font-mono text-sm text-zinc-100 outline-none focus:border-emerald-500"
                    />
                    <InfoTip title="Where recordings are saved">
                      Type any folder on this computer (or leave as-is for the
                      default). A relative path like &quot;recordings&quot; is
                      saved next to the app&apos;s config. The folder is created
